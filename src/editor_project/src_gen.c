...
static uint32_t src_target = SRC_TARGET_ALL;
...
lv_obj_t * screen_manual = NULL;
lv_obj_t * screen_profiles = NULL;
...
const void * icon_thermometer = NULL;
extern const void * icon_thermometer_data;
const void * icon_chart = NULL;
extern const void * icon_chart_data;
const void * icon_layers = NULL;
extern const void * icon_layers_data;
const void * icon_sliders = NULL;
extern const void * icon_sliders_data;
...
lv_style_t style_panel_elevated;
lv_style_t style_text_accent;
lv_style_t style_text_success;
lv_style_t style_text_danger;
lv_style_t style_text_cooling;
lv_style_t style_text_warning;
lv_style_t style_text_secondary;
lv_style_t style_text_muted;
lv_style_t style_hero_temp;
...
lv_subject_t subject_active_tab;
lv_subject_t subject_current_temp;
lv_subject_t subject_target_temp;
lv_subject_t subject_heater_output;
lv_subject_t subject_stage_index;
lv_subject_t subject_heater_active;
lv_subject_t subject_brightness;
lv_subject_t subject_beeper;
lv_subject_t subject_manual_setpoint;
...
void src_init_gen(const char * asset_path)
{
    ...
    if (src_check_target(SRC_TARGET_ALL)) {
        ...
        /* icon_thermometer */
        if (!icon_thermometer) {
            icon_thermometer = &icon_thermometer_data;
        }
        /* icon_chart */
        if (!icon_chart) {
            icon_chart = &icon_chart_data;
        }
        /* icon_layers */
        if (!icon_layers) {
            icon_layers = &icon_layers_data;
        }
        /* icon_sliders */
        if (!icon_sliders) {
            icon_sliders = &icon_sliders_data;
        }
        /* icon_snowflake */
        if (!icon_snowflake) {
    ...
    if (!style_inited) {
        ...
        lv_style_init(&style_panel_elevated);
        lv_style_init(&style_text_accent);
        lv_style_init(&style_text_success);
        lv_style_init(&style_text_danger);
        lv_style_init(&style_text_cooling);
        lv_style_init(&style_text_warning);
        lv_style_init(&style_text_secondary);
        lv_style_init(&style_text_muted);
        lv_style_init(&style_hero_temp);
        lv_style_init(&style_nav_active);
        lv_style_init(&style_btn_stop);
        lv_style_init(&style_scrollbar);

        lv_style_set_bg_color(&style_screen_light, COLOR_DARK_BG);
        lv_style_set_bg_opa(&style_screen_light, (255 * 100 / 100));
        lv_style_set_text_color(&style_screen_light, COLOR_TEXT_PRIMARY);
        ...
        lv_style_set_text_color(&style_screen_dark, COLOR_TEXT_PRIMARY);
        lv_style_set_text_font(&style_screen_dark, font_body);
        lv_style_set_bg_color(&style_panel_light, COLOR_DARK_PANEL);
        lv_style_set_bg_opa(&style_panel_light, (255 * 100 / 100));
        lv_style_set_border_color(&style_panel_light, COLOR_BORDER);
        lv_style_set_border_opa(&style_panel_light, (255 * 100 / 100));
        lv_style_set_border_width(&style_panel_light, BORDER_WIDTH);
        lv_style_set_text_color(&style_panel_light, COLOR_TEXT_PRIMARY);
        lv_style_set_pad_all(&style_panel_light, SPACE_MD);
        lv_style_set_radius(&style_panel_light, RADIUS_CARD);
        lv_style_set_bg_color(&style_panel_dark, COLOR_DARK_PANEL);
        lv_style_set_bg_opa(&style_panel_dark, (255 * 100 / 100));
        lv_style_set_border_color(&style_panel_dark, COLOR_BORDER);
        lv_style_set_border_opa(&style_panel_dark, (255 * 100 / 100));
        lv_style_set_border_width(&style_panel_dark, BORDER_WIDTH);
        lv_style_set_text_color(&style_panel_dark, COLOR_TEXT_PRIMARY);
        lv_style_set_pad_all(&style_panel_dark, SPACE_MD);
        lv_style_set_radius(&style_panel_dark, RADIUS_CARD);
        lv_style_set_bg_color(&style_panel_elevated, COLOR_SURFACE_ELEVATED);
        lv_style_set_bg_opa(&style_panel_elevated, (255 * 100 / 100));
        lv_style_set_border_color(&style_panel_elevated, COLOR_BORDER);
        lv_style_set_border_opa(&style_panel_elevated, (255 * 100 / 100));
        lv_style_set_border_width(&style_panel_elevated, BORDER_WIDTH);
        lv_style_set_text_color(&style_panel_elevated, COLOR_TEXT_PRIMARY);
        lv_style_set_pad_all(&style_panel_elevated, SPACE_MD);
        lv_style_set_radius(&style_panel_elevated, RADIUS_CARD);
        lv_style_set_text_color(&style_text_accent, COLOR_ACCENT);
        lv_style_set_text_color(&style_text_success, COLOR_SUCCESS);
        lv_style_set_text_color(&style_text_danger, COLOR_DANGER);
        lv_style_set_text_color(&style_text_cooling, COLOR_COOLING);
        lv_style_set_text_color(&style_text_warning, COLOR_WARNING);
        ...
        lv_style_set_text_color(&style_hero_temp, COLOR_ACCENT);
        lv_style_set_text_font(&style_hero_temp, font_h1);
        lv_style_set_bg_color(&style_nav_active, COLOR_SURFACE_ELEVATED);
        lv_style_set_border_color(&style_nav_active, COLOR_ACCENT);
        lv_style_set_border_width(&style_nav_active, 1);
        lv_style_set_radius(&style_nav_active, RADIUS_DEFAULT);
        lv_style_set_bg_color(&style_btn_stop, COLOR_DANGER);
        lv_style_set_text_color(&style_btn_stop, COLOR_ACCENT_TEXT);
        lv_style_set_radius(&style_btn_stop, RADIUS_DEFAULT);
    ...
    lv_subject_init_int(&subject_active_tab, 0);
    lv_subject_set_min_value_int(&subject_active_tab, 0);
    lv_subject_set_max_value_int(&subject_active_tab, 4);
    lv_subject_init_int(&subject_current_temp, 183);
    lv_subject_set_min_value_int(&subject_current_temp, 0);
    lv_subject_set_max_value_int(&subject_current_temp, 300);
    lv_subject_init_int(&subject_target_temp, 217);
    lv_subject_set_min_value_int(&subject_target_temp, 0);
    lv_subject_set_max_value_int(&subject_target_temp, 300);
    lv_subject_init_int(&subject_heater_output, 72);
    lv_subject_set_min_value_int(&subject_heater_output, 0);
    lv_subject_set_max_value_int(&subject_heater_output, 100);
    lv_subject_init_int(&subject_stage_index, 2);
    lv_subject_set_min_value_int(&subject_stage_index, 0);
    lv_subject_set_max_value_int(&subject_stage_index, 4);
    lv_subject_init_int(&subject_heater_active, 1);
    lv_subject_set_min_value_int(&subject_heater_active, 0);
    lv_subject_set_max_value_int(&subject_heater_active, 1);
    lv_subject_init_int(&subject_brightness, 85);
    lv_subject_set_min_value_int(&subject_brightness, 10);
    lv_subject_set_max_value_int(&subject_brightness, 100);
    lv_subject_init_int(&subject_beeper, 1);
    lv_subject_set_min_value_int(&subject_beeper, 0);
    lv_subject_set_max_value_int(&subject_beeper, 1);
    lv_subject_init_int(&subject_manual_setpoint, 180);
    lv_subject_set_min_value_int(&subject_manual_setpoint, 0);
    lv_subject_set_max_value_int(&subject_manual_setpoint, 250);
    ...
    lv_xml_register_subject(NULL, "subject_active_tab", &subject_active_tab);
    lv_xml_register_subject(NULL, "subject_current_temp", &subject_current_temp);
    lv_xml_register_subject(NULL, "subject_target_temp", &subject_target_temp);
    lv_xml_register_subject(NULL, "subject_heater_output", &subject_heater_output);
    lv_xml_register_subject(NULL, "subject_stage_index", &subject_stage_index);
    lv_xml_register_subject(NULL, "subject_heater_active", &subject_heater_active);
    lv_xml_register_subject(NULL, "subject_brightness", &subject_brightness);
    lv_xml_register_subject(NULL, "subject_beeper", &subject_beeper);
    lv_xml_register_subject(NULL, "subject_manual_setpoint", &subject_manual_setpoint);
    ...
    lv_xml_register_image(NULL, "icon_thermometer", icon_thermometer);
    lv_xml_register_image(NULL, "icon_chart", icon_chart);
    lv_xml_register_image(NULL, "icon_layers", icon_layers);
    lv_xml_register_image(NULL, "icon_sliders", icon_sliders);
...
{
    return (src_target & target) ? true : false;
}
